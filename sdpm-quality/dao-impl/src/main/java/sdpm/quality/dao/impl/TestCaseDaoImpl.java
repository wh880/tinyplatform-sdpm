...
import static sdpm.quality.dao.inter.org.tinygroup.alm.quality.constant.TestCaseTable.*;
...
import sdpm.quality.dao.inter.org.tinygroup.alm.quality.pojo.TestCase;
import sdpm.quality.dao.inter.org.tinygroup.alm.quality.TestCaseDao;
		...
		return getDslTemplate().insertAndReturnKey(testCase, new InsertGenerateCallback<TestCase>() {
			public Insert generate(TestCase t) {
				Insert insert = insertInto(TESTCASETABLE).values(
					...
					TESTCASETABLE.KEYWORDS.value(t.getKeywords()),
					...
					TESTCASETABLE.CASE_HOWRUN.value(t.getCaseHowRun()),
					...
					TESTCASETABLE.CASE_LINK.value(t.getCaseLink()),
		...
		return getDslTemplate().query(testCase, new SelectGenerateCallback<TestCase>() {
			...
			public Select generate(TestCase t) {
				...
				and(
					...
					TESTCASETABLE.KEYWORDS.eq(t.getKeywords()),
					...
					TESTCASETABLE.CASE_HOWRUN.eq(t.getCaseHowRun()),
					...
					TESTCASETABLE.CASE_LINK.eq(t.getCaseLink()),
		...
		return getDslTemplate().batchInsert(autoGeneratedKeys, testCases, new NoParamInsertGenerateCallback() {
			...
			public Insert generate() {
				return insertInto(TESTCASETABLE).values(
					...
					TESTCASETABLE.KEYWORDS.value(new JdbcNamedParameter("keywords")),
					...
					TESTCASETABLE.CASE_HOWRUN.value(new JdbcNamedParameter("caseHowRun")),
					...
					TESTCASETABLE.CASE_LINK.value(new JdbcNamedParameter("caseLink")),
	...
	public int[] batchDelete(List<TestCase> testCases) {
		...
		return getDslTemplate().batchDelete(testCases, new NoParamDeleteGenerateCallback() {
			...
			public Delete generate() {
				...
				TESTCASETABLE.KEYWORDS.eq(new JdbcNamedParameter("keywords")),
				...
				TESTCASETABLE.CASE_HOWRUN.eq(new JdbcNamedParameter("caseHowRun")),
				...
				TESTCASETABLE.CASE_LINK.eq(new JdbcNamedParameter("caseLink")),
