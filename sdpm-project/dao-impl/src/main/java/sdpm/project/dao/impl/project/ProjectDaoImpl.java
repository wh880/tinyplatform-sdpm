...
package sdpm.project.dao.impl.project;

import static org.tinygroup.tinysqldsl.base.StatementSqlBuilder.and;
import static sdpm.project.dao.inter.project.constant.ProjectTable.*;
...
import sdpm.project.dao.inter.project.pojo.Project;
import sdpm.project.dao.inter.project.ProjectDao;
		...
		return getDslTemplate().insertAndReturnKey(project, new InsertGenerateCallback<Project>() {
			public Insert generate(Project t) {
				Insert insert = insertInto(PROJECTTABLE).values(
					...
					PROJECTTABLE.SLA_CLOSED_BY.value(t.getSlaClosedBy()),
					PROJECTTABLE.SLA_CLOSE_DATE.value(t.getSlaCloseDate()),
		...
		return getDslTemplate().query(project, new SelectGenerateCallback<Project>() {
			...
			public Select generate(Project t) {
				...
				and(
					...
					PROJECTTABLE.SLA_CLOSED_BY.eq(t.getSlaClosedBy()),
					PROJECTTABLE.SLA_CLOSE_DATE.eq(t.getSlaCloseDate()),
		...
		return getDslTemplate().batchInsert(autoGeneratedKeys, projects, new NoParamInsertGenerateCallback() {
			...
			public Insert generate() {
				return insertInto(PROJECTTABLE).values(
					PROJECTTABLE.PROJECT_ID.value(new JdbcNamedParameter("projectId")),
					...
					PROJECTTABLE.SLA_CLOSED_BY.value(new JdbcNamedParameter("slaClosedBy")),
					PROJECTTABLE.SLA_CLOSE_DATE.value(new JdbcNamedParameter("slaCloseDate")),
	...
	public int[] batchDelete(List<Project> projects) {
		...
		return getDslTemplate().batchDelete(projects, new NoParamDeleteGenerateCallback() {
			...
			public Delete generate() {
				...
				PROJECTTABLE.SLA_CLOSED_BY.eq(new JdbcNamedParameter("slaClosedBy")),
				PROJECTTABLE.SLA_CLOSE_DATE.eq(new JdbcNamedParameter("slaCloseDate")),
