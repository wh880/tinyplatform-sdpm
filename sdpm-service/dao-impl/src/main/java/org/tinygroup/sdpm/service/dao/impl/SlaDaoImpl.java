...
import static org.tinygroup.sdpm.service.dao.constant.SlaTable.*;
...
public class SlaDaoImpl extends TinyDslDaoSupport implements SlaDao {
	...
	public Sla add(Sla sla) {
		return getDslTemplate().insertAndReturnKey(sla, new InsertGenerateCallback<Sla>() {
			public Insert generate(Sla t) {
				Insert insert = insertInto(SLATABLE).values(
					...
					SLATABLE.CILENT_PRODUCT_VISION.value(t.getCilentProductVision()));
				return insert;
			}
		});
	}

	public int edit(Sla sla) {
		if(sla == null || sla.getSlaId() == null){
			return 0;
		}
		return getDslTemplate().update(sla, new UpdateGenerateCallback<Sla>() {
			public Update generate(Sla t) {
				Update update = update(SLATABLE).set(
					...
					SLATABLE.SLA_OPEN_COUNT.value(t.getSlaOpenCount()),
					SLATABLE.DELETED.value(t.getDeleted()),
					SLATABLE.CILENT_PRODUCT_VISION.value(t.getCilentProductVision())).where(
	...
	public Sla getByKey(Integer pk) {
		return getDslTemplate().getByKey(pk, Sla.class, new SelectGenerateCallback<Serializable>() {
	...
	public List<Sla> query(Sla sla ,final OrderBy... orderBies) {
		...
		return getDslTemplate().query(sla, new SelectGenerateCallback<Sla>() {
		...
		return getDslTemplate().batchInsert(autoGeneratedKeys, slas, new NoParamInsertGenerateCallback() {
			...
			public Insert generate() {
				return insertInto(SLATABLE).values(
					...
					SLATABLE.CILENT_PRODUCT_VISION.value(new JdbcNamedParameter("cilentProductVision")));
			}
		});
	}
		...
		return getDslTemplate().batchUpdate(slas, new NoParamUpdateGenerateCallback() {
			...
			public Update generate() {
				return update(SLATABLE).set(
					...
					SLATABLE.SLA_OPEN_COUNT.value(new JdbcNamedParameter("slaOpenCount")),
					SLATABLE.DELETED.value(new JdbcNamedParameter("deleted")),
					SLATABLE.CILENT_PRODUCT_VISION.value(new JdbcNamedParameter("cilentProductVision"))).where(
	...
	public int[] batchDelete(List<Sla> slas) {
		...
		return getDslTemplate().batchDelete(slas, new NoParamDeleteGenerateCallback() {
			...
			public Delete generate() {
				...
				SLATABLE.CILENT_PRODUCT_VISION.eq(new JdbcNamedParameter("cilentProductVision"))));
			}
		});
	}
