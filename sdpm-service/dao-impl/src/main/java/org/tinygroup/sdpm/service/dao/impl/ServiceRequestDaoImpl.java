...
public class ServiceRequestDaoImpl extends TinyDslDaoSupport implements ServiceRequestDao {
    ...
    public Pager<ServiceRequest> queryPagerBy(int start, int limit, ServiceRequest serviceRequest, final Integer statues, final Integer treeId, final Condition condition1, final OrderBy... orderArgs) {
        if (serviceRequest == null) {
            serviceRequest = new ServiceRequest();
        }
        return getDslTemplate().queryPager(start, limit, serviceRequest, false, new SelectGenerateCallback<ServiceRequest>() {
    ...
    public List<ServiceRequest> requestInCondition(String condition, Integer limit) {
        Select select = MysqlSelect.selectFrom(SERVICE_REQUESTTABLE).where(
                and(SERVICE_REQUESTTABLE.REQUEST_TITLE.like(condition), SERVICE_REQUESTTABLE.DELETED.eq(0))).limit(0, limit);
