...
import static org.tinygroup.sdpm.service.constant.RequestTable.*;
...
import org.tinygroup.sdpm.service.inter.RequestDao;
...
public class RequestDaoImpl extends TinyDslDaoSupport implements RequestDao {
	...
	public Request insertObject(Request request) {
		return getDslTemplate().insertObject(request, new InsertGenerateCallback<Request>() {
			public Insert generate(Request t) {
				Insert insert = insertInto(REQUESTTABLE).values(
					REQUESTTABLE.REQUEST_ID.value(t.getRequestId()),
					REQUESTTABLE.COMPANY_ID.value(t.getCompanyId()),
					REQUESTTABLE.PRODUCT_ID.value(t.getProductId()),
					REQUESTTABLE.MODULE_ID.value(t.getModuleId()),
					REQUESTTABLE.REQUEST_N_O.value(t.getRequestNO()),
					...
					REQUESTTABLE.RELEASE_ID.value(t.getReleaseId()),
	...
	public Request insertObject(boolean autoGeneratedKeys ,Request request) {
		return getDslTemplate().insertObjectAndReturnKey(autoGeneratedKeys ,request, new InsertGenerateCallback<Request>() {
	...
	public int updateObject(Request request) {
		return getDslTemplate().updateObject(request, new UpdateGenerateCallback<Request>() {
			public Update generate(Request t) {
				Update update = update(REQUESTTABLE).set(
					...
					REQUESTTABLE.REQUEST_ID.eq(t.getRequestId()));
				return update;
			}
		});
	}
	...
	public int deleteObject(Serializable pk){
		return getDslTemplate().deleteObject(pk, new DeleteGenerateCallback<Serializable>() {
			public Delete generate(Serializable pk) {
				return delete(REQUESTTABLE).where(REQUESTTABLE.REQUEST_ID.eq(pk));
			}
		});
	}
	...
	public int deleteObjects(Serializable... pks) {
		return getDslTemplate().deleteObjects(new DeleteGenerateCallback<Serializable[]>() {
			public Delete generate(Serializable[] t) {
				return delete(REQUESTTABLE).where(REQUESTTABLE.REQUEST_ID.in(t));
		}
		},pks);
	}

	public Request getObjectById(Serializable pk) {
		return getDslTemplate().getObjectById(pk, Request.class, new SelectGenerateCallback<Serializable>() {
		@SuppressWarnings("rawtypes")
		public Select generate(Serializable t) {
			return selectFrom(REQUESTTABLE).where(REQUESTTABLE.REQUEST_ID.eq(t));
			}
		});
	}

	public List<Request> queryObjects(Request request) {
		if(request==null){
			request=new Request();
		}
		return getDslTemplate().queryObjects(request, new SelectGenerateCallback<Request>() {

			@SuppressWarnings("rawtypes")
			public Select generate(Request t) {
				return selectFrom(REQUESTTABLE).where(
				and(
					REQUESTTABLE.COMPANY_ID.eq(t.getCompanyId()),
					REQUESTTABLE.PRODUCT_ID.eq(t.getProductId()),
					REQUESTTABLE.MODULE_ID.eq(t.getModuleId()),
					REQUESTTABLE.REQUEST_N_O.eq(t.getRequestNO()),
					...
					REQUESTTABLE.RELEASE_ID.eq(t.getReleaseId()),
	...
	public Pager<Request> queryObjectsForPage(int start,int limit ,Request request) {
		if(request==null){
			request=new Request();
		}
		return getDslTemplate().queryObjectsForPage(start, limit, request, false, new SelectGenerateCallback<Request>() {
		...
		return getDslTemplate().batchInsert(autoGeneratedKeys, requests, new NoParamInsertGenerateCallback() {
			...
			public Insert generate() {
				return insertInto(REQUESTTABLE).values(
					REQUESTTABLE.COMPANY_ID.value(new JdbcNamedParameter("companyId")),
					...
					REQUESTTABLE.REQUEST_N_O.value(new JdbcNamedParameter("requestNO")),
					...
					REQUESTTABLE.RELEASE_ID.value(new JdbcNamedParameter("releaseId")),
	...
	public int[] batchUpdate(List<Request> requests) {
		...
		return getDslTemplate().batchUpdate(requests, new NoParamUpdateGenerateCallback() {
			...
			public Update generate() {
				...
				REQUESTTABLE.REQUEST_ID.eq(new JdbcNamedParameter("requestId")));
			}
		});
	}
	...
	public int[] batchDelete(List<Request> requests) {
		...
		return getDslTemplate().batchDelete(requests, new NoParamDeleteGenerateCallback() {
			...
			public Delete generate() {
				...
				REQUESTTABLE.REQUEST_ID.eq(new JdbcNamedParameter("requestId")),
				REQUESTTABLE.COMPANY_ID.eq(new JdbcNamedParameter("companyId")),
				REQUESTTABLE.PRODUCT_ID.eq(new JdbcNamedParameter("productId")),
				REQUESTTABLE.MODULE_ID.eq(new JdbcNamedParameter("moduleId")),
				REQUESTTABLE.REQUEST_N_O.eq(new JdbcNamedParameter("requestNO")),
				...
				REQUESTTABLE.RELEASE_ID.eq(new JdbcNamedParameter("releaseId")),
