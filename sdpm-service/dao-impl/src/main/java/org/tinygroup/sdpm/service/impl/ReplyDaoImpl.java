...
import static org.tinygroup.sdpm.service.constant.ReplyTable.*;
...
import org.tinygroup.sdpm.service.inter.ReplyDao;
...
public class ReplyDaoImpl extends TinyDslDaoSupport implements ReplyDao {
	...
	public Reply insertObject(Reply reply) {
		return getDslTemplate().insertObject(reply, new InsertGenerateCallback<Reply>() {
			public Insert generate(Reply t) {
				Insert insert = insertInto(REPLYTABLE).values(
					REPLYTABLE.REPLY_ID.value(t.getReplyId()),
					REPLYTABLE.REQUEST_ID.value(t.getRequestId()),
	...
	public Reply insertObject(boolean autoGeneratedKeys ,Reply reply) {
		return getDslTemplate().insertObjectAndReturnKey(autoGeneratedKeys ,reply, new InsertGenerateCallback<Reply>() {
	...
	public int updateObject(Reply reply) {
		return getDslTemplate().updateObject(reply, new UpdateGenerateCallback<Reply>() {
	...
	public Reply getObjectById(Serializable pk) {
		return getDslTemplate().getObjectById(pk, Reply.class, new SelectGenerateCallback<Serializable>() {
	...
	public List<Reply> queryObjects(Reply reply) {
		if(reply==null){
			reply=new Reply();
		}
		return getDslTemplate().queryObjects(reply, new SelectGenerateCallback<Reply>() {

			@SuppressWarnings("rawtypes")
			public Select generate(Reply t) {
				return selectFrom(REPLYTABLE).where(
				and(
					REPLYTABLE.REQUEST_ID.eq(t.getRequestId()),
	...
	public Pager<Reply> queryObjectsForPage(int start,int limit ,Reply reply) {
		if(reply==null){
			reply=new Reply();
		}
		return getDslTemplate().queryObjectsForPage(start, limit, reply, false, new SelectGenerateCallback<Reply>() {
		...
		return getDslTemplate().batchInsert(autoGeneratedKeys, replys, new NoParamInsertGenerateCallback() {
			...
			public Insert generate() {
				return insertInto(REPLYTABLE).values(
					REPLYTABLE.REQUEST_ID.value(new JdbcNamedParameter("requestId")),
	...
	public int[] batchDelete(List<Reply> replys) {
		...
		return getDslTemplate().batchDelete(replys, new NoParamDeleteGenerateCallback() {
			...
			public Delete generate() {
				...
				REPLYTABLE.REPLY_ID.eq(new JdbcNamedParameter("replyId")),
				REPLYTABLE.REQUEST_ID.eq(new JdbcNamedParameter("requestId")),
